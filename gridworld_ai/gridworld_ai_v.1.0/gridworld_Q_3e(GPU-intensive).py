                ...
                for y2 in range(-SIZE+1, SIZE):
                    for x3 in range(-SIZE+1, SIZE):
                        for y3 in range(-SIZE+1, SIZE):
                            Q[((x1, y1), (x2, y2), (x3, y3))] = [np.random.uniform(-ACTION_SPACE, 0)
...
for episode in range(EPISODES):
    ...
    for i in range(MAX_STEPS):
        state = (player - food, player - enemy0, player - enemy1)
        ...
        if ENEMY_FOOD_MOVE == True:
            ...
            pass
        new_state = (player - food, player - enemy0, player - enemy1)
        ...
        elif player.x == enemy0.x and player.y == enemy0.y:
        ...
        elif player.x == enemy1.x and player.y == enemy1.y:
            reward = -ENEMY_PENALTY
            Q[state][action] = -ENEMY_PENALTY
            enemy_counter += 1
            enemy_counter_ep += 1
